...
--
--
-- <*> Description = A - Permissions (Active Screens)
...
FROM
	Assessments[#SFX#].dbo.tblPermissions P
	LEFT OUTER JOIN Assessments[#SFX#].dbo.tblRole R ON (R.RoleID = P.RoleID)
	LEFT OUTER JOIN Assessments[#SFX#].dbo.codePermissionType PT ON (PT.PermTypeID = P.PermTypeID)
	LEFT OUTER JOIN Assessments[#SFX#].dbo.tblObject O ON (O.ObjectID = P.ObjectID)
	LEFT OUTER JOIN Assessments[#SFX#].dbo.tblObject PO ON (PO.ObjectID = O.ParentObjectID)
	LEFT OUTER JOIN Assessments[#SFX#].dbo.codeScreen S ON (S.ScreenID = O.ScreenID)
